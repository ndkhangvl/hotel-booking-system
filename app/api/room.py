...
async def initialize(
    ...
    """
    try:
        return crud_room.get_initialize_stats(branch_code=branch_code, start_date=start_date, end_date=end_date)
    except Exception as e:
...
async def rooms_list(
    branch_code: str = Query(..., description="ID của chi nhánh"),
    start_date: date | None = Query(default=None, description="Ngày bắt đầu xem trạng thái phòng"),
    end_date: date | None = Query(default=None, description="Ngày kết thúc xem trạng thái phòng"),
    ...
    """
    try:
        return crud_room.get_rooms_by_branch(branch_code, page, page_size, active_only=False, start_date=start_date, end_date=end_date)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Lỗi lấy danh sách phòng: {e}")
...
async def branch_rooms_list(
    ...
    try:
        return crud_room.get_branch_rooms_by_branch(branch_code, page, page_size, start_date=start_date, end_date=end_date)
    except Exception as e:
